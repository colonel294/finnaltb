...
def filters(bot: Bot, update: Update):
    ...
    if len(extracted) >= 2:
        ...
        if not content:
            msg.reply_text("اومم یه چیزی باید برام مشخص کنیکه بهش جواب بدم . دستورت کامل نیست")
            return
...
__help__ = """
فیلتر ها میتونن خیلی مفید باشن . من از طریق فیلتر ها میتونم به کسی که چیزی خواست ارسال کنم، یا به پیامش جواب خاصی بدم 
چون سرورم زیاد بزرگ نیست ! ممکنه گاهی فیلتر های تعریف شدم پاک بشه و از اول بیاد .
میتونید بهم تو بخش /donate کمک کنید یاا به سازندم بگید یه کپی ازم برای شخص خودتون بسازه که با این مشکل برنخورید!

 - /filters:همه فیلتر های فعال در این گپ

*فقط ادمینها:*
 - /filter <keyword> <reply>: 
 اومم با این دستور یه فیلتر جدید اضافه میکنی ! تو قسمت keyword متنی که میخوای بهش جواب بدم رو مینویسی 
 و تو قسمت reply جوابی که قراره بهش بدم رو .اگه میخوای جواب به صورت فایل یا استیکر یا ... باشه ، روی اون فایلت ریپلی کن 
 فقط برام keywordرو بنویس تا زمانی که من متن رو دیدم براش فایلی که گفتی رو بفرستم
 - /stop <keyword>: دیگه به اون پیامی که گفتی جواب نمیدم!
"""

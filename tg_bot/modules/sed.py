...
def sed(bot: Bot, update: Update):
    ...
    if sed_result and update.effective_message.reply_to_message:
        ...
        if not repl:
            update.effective_message.reply_to_message.reply_text("تو میخوای هیچیو  "
                                                                 "بزاری جای چیزی؟ .من گیج شدم")
            return
        ...
        try:
            ...
            if check and check.group(0).lower() == to_fix.lower():
                update.effective_message.reply_to_message.reply_text("آهای ملت, {} میخواد کاری کنه "
                                                                     "که من چیزاییو بگم که  "
                                                                     "که دوس ندارم".format(update.effective_user.first_name))
                return
            ...
            LOGGER.exception("SRE  error")
            update.effective_message.reply_text("اومم من نمیفهمم اینو . به سازندم بگو.")
            return

        # empty string errors -_-
        if len(text) >= telegram.MAX_MESSAGE_LENGTH:
            update.effective_message.reply_text("متنی که نوشتی خیلی طولانیه برا  \
                                                 تلگرام")
        elif text:
...
__help__ = """
اومم این یه قابلیت برای حذف پیام های بد
 و جایگزاری حرف های مثبت تر به جای اونهاس .
 هنوز کامل نشده چون هاستی که دارم به حد کافی قدرمتند نیست

""".format(telegram.MAX_MESSAGE_LENGTH)

__mod_name__ = "گشت ارشاد"

    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("تمام {} برای افراد عادی قفل شد!".format(args[0]))
                ...
                message.reply_text("چیو میخوای قفل کنی متوجه نمیشم ! از دستور /locktypes برای قفل های موجور استفاده کن!")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("{} ها برای همه باز شد!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
            ...
            if lockable == "bots":
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        ...
                        message.reply_text("فقط ادمینا اجازه دارن ربات بیارن !! گمشو بیروون")
            else:
                try:
                    message.delete()
...
__help__ = """
 - /locktypes: لیست قفل هایی که من دارم رو نشون میدم

*Admin only:*
 - /lock <type>: در قسمت type .بخش مورد نظر رو بنویسید من قفلش میکنم
 - /unlock <type>: در قسمت type بخش مورد نظر رو بنویسید من بازش میکنم 
 - /locks: لیستی از قفل هایی که اعمال شده!.

قفل ها میتونن برای محدود کردن کاربر ها استفاده بشن.
نکته:
قفل لینک یا همون url .باعث میشه هر لینکی داخل گپتون بود رو پاک کنم  \
قفل استیکر هم همینطور.
قفل ربات ها باعث میشه کاربران عادی نتونن رباتی اضافه کنن.
"""

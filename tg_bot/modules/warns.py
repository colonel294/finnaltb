...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} اخطار شد, {} از گروه اخراج شد🎚".format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "{} اخطار شد, {} از گروه بن شد🎚".format(limit, mention_html(user.id, user.first_name))
...
def set_warn_limit(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if args:
        ...
        msg.reply_text("در حال حاضر تعداداخطار {} هست!".format(limit))
    return ""
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        ...
        limit, soft_warn = sql.get_warn_setting(chat.id)
        if soft_warn:
            msg.reply_text("حالت سخت گیرانه خاموشه، کسایی که خیلی اخطار بگیرن فقط اخراج میشن.",
                           parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text("حالت سختگیرانه خاموشه ، کسایی که خیلی اخطار بگیرن اخراج و بن میشن.",
                           parse_mode=ParseMode.MARKDOWN)
    return ""
...
__help__ = """
 - /warns <آیدی>: 
تعداد و دلایل اخطار اون آیدی رو نشون میدم!.
 - /warnlist: لیست همه کار هایی که بخاطرشون ممکنه اخطار بگیرید رو نشون میدم

*فقط ادمینها:*
 - /warn <آیدی>: 
 به ایدی که اشاره کردی اخطار میدم . وقتی به حد اخطار هاش برسه از گروه اخراج میشه ، میتونی روش هم ریپلی بزنی!.
 - /resetwarn <آیدی>: 
 اخطار هایی که گرفته  رو صفر میکنم ، میتونی روش ریپلی هم بزنی!.
 - /addwarn <دلیل> <کلمه یا جمله>: 
 به کسی که کلمه یا جمله رو بنویسه ،مثل فوش، اخطار میدم . به همراه دلیل ! \
اگه میخوای مثلا یه جمله رو جزو اخطار نامه اضافه کنی میتونی مثل این عمل کنی: 
/addwarn " آدم مزخرف" . 
 - /nowarn <کلمه یا جمله>: 
دیگه به اون کلمه یا جمله اخطار نمیدم!
 - /warnlimit <عدد>: 
تعداد اخطار هارو مشخص میکنی برام . کاربرات به   تعداد تایین شده برسن اخراجشون میکنم
 - /strongwarn <on/yes/off/no>: حالت سختگیرانه ، وقتی روشنه شیطونا رو هم اخراج و هم بن میکنم. در غیر اینصورت فقط اخراج.
"""
